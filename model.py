...
from DCSP import CoordAttMeanMax  # 导入DCSP模块
from LFGP import LRSA  # 导入LRSA模块
...
class MobileViT(nn.Module):
    ...
    def __init__(self, model_cfg: Dict, num_classes: int = 1000):
        ...
        # 为第三、四层创建LRSA模块
        self.lrsa_layer3 = LRSA(
            dim=model_cfg["layer3"]["out_channels"],
            qk_dim=model_cfg["layer3"]["transformer_channels"],
            mlp_dim=model_cfg["layer3"]["ffn_dim"],
            heads=model_cfg["layer3"]["num_heads"]
        )

        self.layer_3, out_channels = self._make_layer(input_channel=out_channels, cfg=model_cfg["layer3"],
                                                      layer_name="layer3")

        self.lrsa_layer4 = LRSA(
            dim=model_cfg["layer4"]["out_channels"],
            qk_dim=model_cfg["layer4"]["transformer_channels"],
            mlp_dim=model_cfg["layer4"]["ffn_dim"],
            heads=model_cfg["layer4"]["num_heads"]
        )
    ...
    def _make_layer(self, input_channel, cfg: Dict, layer_name: str) -> Tuple[nn.Sequential, int]:
        block_type = cfg.get("block_type", "mobilevit")
        if block_type.lower() == "mobilevit":
            return self._make_mit_layer(input_channel=input_channel, cfg=cfg, layer_name=layer_name)
        else:
            return self._make_mobilenet_layer(input_channel=input_channel, cfg=cfg, layer_name=layer_name)

    def _make_mobilenet_layer(self, input_channel: int, cfg: Dict, layer_name: str) -> Tuple[nn.Sequential, int]:
        ...
        for i in range(num_blocks):
            ...
            # 在第二层的第一个MV2↓模块后插入DCSP模块
            if layer_name == "layer2" and i == 0 and stride == 2:
                block.append(CoordAttMeanMax(inp=input_channel, oup=input_channel))

        return nn.Sequential(*block), input_channel

    def _make_mit_layer(self, input_channel: int, cfg: Dict, layer_name: str) -> [nn.Sequential, int]:
    ...
    def forward(self, x: Tensor) -> Tensor:
        x = self.conv_1(x)
        x = self.layer_1(x)
        x = self.layer_2(x)  # 已包含DCAFE模块

        # 处理第三层：MV2↓ -> LRSA(ps=8) -> MobileViTBlock
        x = self.layer_3[0](x)  # MV2↓模块
        x = self.lrsa_layer3(x, ps=10)  # LRSA模块，ps=8
        x = self.layer_3[1](x)  # MobileViTBlock模块

        # 处理第四层：MV2↓ -> LRSA(ps=4) -> MobileViTBlock
        x = self.layer_4[0](x)  # MV2↓模块
        x = self.lrsa_layer4(x, ps=6)  # LRSA模块，ps=4
        x = self.layer_4[1](x)  # MobileViTBlock模块
